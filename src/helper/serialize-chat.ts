...
export const serializeMarkdown = (tabId: string): string => {
  return (MynahUITabsStore.getInstance().getTabDataStore(tabId).getValue('chatItems') as ChatItem[]).map(chatItem => escapeHtml(chatItem.body ?? '')).filter(chatItem => chatItem.trim() !== '').join('\n\n---\n\n') ?? '';
};
