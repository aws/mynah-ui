    ...
    onChatPrompt: (tabId: string, prompt: ChatPrompt) => {
      if (tabId === 'tab-1') {
        mynahUI.updateStore(tabId, {
          tabCloseConfirmationMessage: `Working on "${prompt.prompt != null ? (prompt.prompt as string) : ''}"`,
        });
      }
      ...
      switch (prompt.command) {
        ...
        default:
          mynahUI.addChatItem(tabId, {
            ...
            messageId: generateUID(),
            body: `**${(prompt.command != null ? prompt.command as string : '').replace('/', '')}**\n${(prompt.escapedPrompt != null ? prompt.escapedPrompt as string : '')}`,
          });
