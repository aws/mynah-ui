...
import * as BaseConfig from './base-theme-config.json';
...
export class ThemeBuilder {
    ...
    private currentConfig: Record<any, ConfigItem> = structuredClone(BaseConfig) as any;
    constructor(selector: string | HTMLElement) {
        ...
        resetThemeConfigButton.addEventListener('click', () => {
            this.currentConfig = structuredClone(BaseConfig) as Record<any, ConfigItem>;
            this.inputsWrapper.innerHTML = "";
            this.fillInputWrapper();
            this.buildCssValues();
        });
        ...
        downloadThemeButton.addEventListener('click', () => {
            download('mynah-ui-theme.css', `:root {
    ...
    private fillInputWrapper = () => {
        categories.forEach(category => {
            this.inputsWrapper.insertAdjacentHTML('beforeend', `
        ...
        Object.keys(this.currentConfig).forEach((themeConfigKey: string) => {
            ...
            switch (themeConfigItem.type) {
                case 'text':
                    this.inputsWrapper.insertAdjacentElement('beforeend', themeInputText(
                        themeConfigKey,
                        themeConfigItem,
                        (value) => {
                    ...
                    break;
                case 'measurement':
                    this.inputsWrapper.insertAdjacentElement('beforeend', themeInputMeasurement(
                        themeConfigKey,
                        themeConfigItem,
                        (value, unit) => {
                            this.currentConfig[themeConfigKey].value = value;
                    ...
                    break;
                case 'color':
                    this.inputsWrapper.insertAdjacentElement('beforeend', themeInputColor(
                        themeConfigKey,
                        themeConfigItem,
                        (hex, alpha) => {
    ...
    private getCssCustomVars = (): string => Object.keys(this.currentConfig).map(configKey => {
...
const themeInputMeasurement = (title: string, configItem: ConfigItem, onValueChange: (value: string, unit: string) => void): HTMLDivElement => {
    ...
    configItem.units?.forEach(unitKey => {
        selectElement.insertAdjacentHTML('beforeend', `
            <option ${configItem.unit === unitKey ? 'selected' : ''} value="${unitKey}">${unitKey !== '' ? unitKey : '(No Unit)'}</option>
            `);
    });
...
const themeInputColor = (title: string, configItem: ConfigItem, onValueChange: (hex: string, alpha: string) => void): HTMLDivElement => {
    ...
    inputElementLabelWrapper.insertAdjacentHTML('beforeend', `
