...
Here's a preview the refactored files (If there is only one file, you can just link to the file or try to generate a base64 image for the preview and put it here):
...
export const exampleImageCard = ():ChatItem => {
  return {
    messageId: (new Date().getTime()).toString(),
    type: ChatItemType.ANSWER,
    body: `Here's a QR code for mynah-ui github link:
